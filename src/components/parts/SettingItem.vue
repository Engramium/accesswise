...
<template>
	<div class="setting-wrap">
		...
		<el-checkbox-group class="setting-input" v-if="content.input == 'checkbox'" :model-value="modelValue" @change="changeSetting($event, content)">
			<div v-for="(item, key) in content.options" :key="key">
				<el-checkbox @click="currentClick = key" :label="item" :value="key" />
			</div>
		</el-checkbox-group>

		<el-select v-if="content.input == 'select'" class="m-2" placeholder="Select" size="large" style="width: 240px" :model-value="modelValue" @change="changeSetting($event, content)">
