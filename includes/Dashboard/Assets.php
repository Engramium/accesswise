...
class Assets {
	...
	public function enqueue_scripts() {
		...
		if ( 'toplevel_page_accesswise-settings' != $screen->id ) {
			return;
		}
	...
	public function enqueue_frontend_scripts() {
		wp_enqueue_script( 'accesswise-frontend' );
		$i18n = [
			'rest_url'   => rest_url( '/' ),
			'admin_ajax' => admin_url( 'admin-ajax.php' ),
			'nonce'      => wp_create_nonce( 'accesswise_public_nonce' )
		];
		$i18n = apply_filters( 'accesswise/frontend/i18n', $i18n );
		wp_localize_script( 'accesswise-frontend', 'accesswise', $i18n );
	}
